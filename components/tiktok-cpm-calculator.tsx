...
function guidanceForCpm(cpm: number, impressionsPerDollar: number) {
  ...
  if (cpm <= 0) {
    guidance.push("Enter ad spend and impressions to calculate TikTok CPM.");
    return guidance;
  }

  if (cpm > 14) {
    guidance.push(
      "CPM is high for a planning baseline. Check audience size, bid strategy, creative fatigue, and whether the campaign objective is too narrow."
    );
  }

  if (cpm < 6) {
    guidance.push(
      "CPM is efficient. Watch click quality, conversion rate, and retention so cheap reach does not hide weak downstream performance."
    );
  }

  if (impressionsPerDollar < 75) {
    guidance.push(
      "Impressions per dollar are limited. Test broader audiences, fresh hooks, and clearer creative angles before scaling spend."
    );
  }

  if (!guidance.length) {
    guidance.push(
      "CPM is in a workable planning range. Compare it with CPC, CTR, conversion rate, and creator content costs before changing budget."
    );
  }
...
export function TikTokCpmCalculator() {
  ...
  return (
    <section
      ...
      aria-labelledby="cpm-calculator-title"
    >
      <div className="flex flex-col gap-2 border-b border-slate-200 pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 id="cpm-calculator-title" className="mt-2 text-2xl font-bold tracking-normal">
            TikTok CPM snapshot
          </h2>
        </div>
      ...
      <p className="mt-5 text-xs leading-5 text-slate-500">
        CPM is useful for reach planning, but it should not be judged alone. Compare it with CPC,
        CTR, conversion rate, audience quality, and creative fatigue.
      </p>
    </section>
  );
}
